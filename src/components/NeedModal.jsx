...
/**
 * API ISO (usually UTC Z) -> local datetime-local input value "YYYY-MM-DDTHH:mm"
 * IMPORTANT: Must convert to local time (Brisbane) correctly.
 */
function isoToLocalInput(iso) {
  if (!iso) return "";
  const d = new Date(iso); // ISO Z -> Date in local timezone
...
/**
 * datetime-local input "YYYY-MM-DDTHH:mm" -> UTC ISO with Z
 * (interprets the input as LOCAL time, then converts to UTC for API)
 */
function localInputToUtcIso(dtLocal) {
...
export default function NeedModal({
  ...
  const onChange = (e) => {
    ...
    setForm((p) => ({
      ...p,
      [id]: inputType === "number" ? value : value,
    }));
  };
  ...
  const validate = () => {
    ...
    if (type === "money") {
      ...
      if (Number(form.target_amount) <= 0) return "Target amount must be > 0.";
    }
    ...
    if (type === "time") {
      ...
      const startIso = localInputToUtcIso(form.start_datetime);
      const endIso = localInputToUtcIso(form.end_datetime);
      if (!startIso || !endIso) return "Start and end must be valid date/time values.";
      ...
      <div className="modal">
        <div className="modal__head">
          <h3 className="modal__title">
            {isEdit ? `Edit ${prettyType} need` : `Add ${prettyType} need`}
          </h3>
        ...
        <div className="modal__grid">
          ...
          {type === "money" && (
            <>
              <div className="field field--full">
                <label className="field__label" htmlFor="target_amount">
                  Target amount
                </label>
                <input
          ...
          {type === "item" && (
            <>
              <div className="field field--full">
                <label className="field__label" htmlFor="item_name">
                  Item name
                </label>
              ...
              <div className="field">
                <label className="field__label" htmlFor="quantity_needed">
                  Quantity
                </label>
                <input
                  className="field__input"
          ...
          {type === "time" && (
            <>
              <div className="field field--full">
                <label className="field__label" htmlFor="role_title">
                  Role title
                </label>
              ...
              <div className="field">
                <label className="field__label" htmlFor="volunteers_needed">
                  Volunteers
                </label>
                <input
                  className="field__input"
              ...
              <div className="field">
                <label className="field__label" htmlFor="start_datetime">
                  Start
                </label>
                <input
                  className="field__input"
              ...
              <div className="field">
                <label className="field__label" htmlFor="end_datetime">
                  End
                </label>
                <input
                  className="field__input"
